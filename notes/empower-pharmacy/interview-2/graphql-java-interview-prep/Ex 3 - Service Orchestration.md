...
<details>
<summary>Dashboard controller (click to reveal)</summary>
...
public class DashboardController {
    ...
    public Mono<Dashboard> dashboard(@Argument String userId) {
        ...
        Mono<Integer> notifCount = notifClient.getUnreadCount(userId)
            .onErrorReturn(0);  // graceful degradation
        Mono<List<Product>> recs = recClient.getForUser(userId)
            .onErrorReturn(List.of());  // graceful degradation
...
@QueryMapping
public CompletableFuture<Dashboard> dashboard(@Argument String userId) {
    var userF = userClient.getUser(userId);
    var ordersF = orderClient.getRecentOrders(userId, 10);
    var notifF = notifClient.getUnreadCount(userId)
        .exceptionally(ex -> { log.warn("Notif failed", ex); return 0; });
    var recsF = recClient.getForUser(userId)
