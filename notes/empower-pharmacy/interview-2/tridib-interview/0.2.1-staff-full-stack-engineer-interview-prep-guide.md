...
```
- see "0.2.2-event-sourcing-open-telemetry-distributed-tracing" document for event sourcing, open telemetry-distributed tracing

- Each listener is a stateless Kubernetes Deployment with multiple replicas. Service Bus handles message distribution across pods. We use KEDA to autoscale based on subscription queue depth. For ordered processing within a single prescription, we use Service Bus sessions — parallelism is across orders, not within one order. The listeners are idempotent, so duplicate delivery from competing consumers is safe.
```
---
...
**Additional notes:**
- ==Use **Service Bus** when you’re telling _one specific service_ to do something. Use **Kafka** when you’re announcing that something happened and _anyone who cares_ should know about it.==
- for compensating actions, see "0.2.2-saga-orchestration-flow-diagrams-prescription-order-happy-path-failure-with-compensation" document
**Orchestrator State Persistence:**
- Every saga instance is a database row with `status`, `current_step`, and `step_results` (a JSON map of each step’s outcome).
- ==**Write-ahead pattern**: update the DB _before_ sending each command, then again after receiving the response.==
- On crash recovery: query all stuck sagas, check with participant services whether the in-progress step was actually processed (idempotency makes this safe), and resume.
- Practical shortcut: **Temporal** or **Azure Durable Functions** handle all of this automatically — you write a sequential function and the framework persists execution state, replays from checkpoints, and manages retries.
...
``` hl:1-2,11-12,15
Every REST endpoint has a unique URL. HTTP caches (browsers, CDNs, proxies) key on that URL automatically.
// GET /api/users/42, The browser/CDN caches this by URL + headers
HTTP/1.1 200 OK
Cache-Control: max-age=300       // cache for 5 minutes
ETag:          "a1b2c3"            // fingerprint for revalidation
{
  "id": 42,
  "name": "Alice",
  "email": "alice@acme.com"
}
`Cache-Control: max-age=300` — The server tells the browser (and any CDN in between): “This response is valid for 300 seconds. Don’t ask me again until then.” During that window, the browser serves it from its local cache with zero network requests.
`ETag: "a1b2c3"` — A fingerprint of the response content, also set by the server. After `max-age` expires, the browser doesn’t fetch the full response again immediately. Instead it sends a lightweight request:
GET /api/users/42
If-None-Match: "a1b2c3"
The server compares the ETag. If the data hasn’t changed, it responds with `304 Not Modified` (no body) — saving bandwidth. If it has changed, it sends the full new response with a new ETag
```
